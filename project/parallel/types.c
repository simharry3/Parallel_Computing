...
void updateCollision(particle* cols, int numAll){
    MPI_Request request;
    MPI_Irecv(cols, numAll, MPI_INT, , , , MPI_COMM_WORLD, &request)   
}
...
void initContext(context** ctx, int* data, int commsize){
    *ctx = calloc(1, sizeof(context));
    (*ctx)->max = calloc(3, sizeof(int));
    (*ctx)->numParticles = data[0]/commsize;
    (*ctx)->numSteps = data[1];
    (*ctx)->max[0] = 100;
    (*ctx)->max[1] = 100;
    (*ctx)->max[2] = 100;
